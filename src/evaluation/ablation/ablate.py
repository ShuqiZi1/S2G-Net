...
class EdgeDropWrapper(nn.Module):
    ...
    def forward(self, x, flat, adjs, batch_size, edge_weight):
        ...
        for idx, (edge_index, e_id, size) in enumerate(adjs):
            ...
            new_adjs.append((edge_index[:, keep],
                             e_id[keep] if e_id is not None else None,
                             size))
            if edge_weight is not None:
                new_edge_weight.append(edge_weight[keep])
        if edge_weight is not None:
    ...
    def inference(self, x_all, flat_all, edge_weight,
        ...
        sparse_w = edge_weight if edge_weight is not None else None
...
def run_ablation_experiment(model, loader, subgraph_loader, ts_loader,
    ...
    results = {
        'kappa': kappa,
        'mad': mean_absolute_error(truth_np, pred_np),
        'mape': mean_absolute_percentage_error(truth_np, pred_np)*100,
        'mse': mean_squared_error(truth_np, pred_np),
        'msle': mean_squared_log_error(truth_np, pred_np),
        'r2': r2_score(truth_np, pred_np)
    }
