...
from src.explain.run_explain import run_interpretability_experiments
from src.ablation.ablate import run_ablation_experiment, run_all_ablations
...
class MambaGPSModel(pl.LightningModule):
    def __init__(self, config, dataset, train_loader, subgraph_loader, eval_split='test',
        ...
        def loss_fn(pred, y):
            huber = F.huber_loss(torch.log1p(pred.clamp_min(1e-6)),
                                  y, delta=0.5, reduction='none')
            weight = 1.0 + 2.5*(y > 14).float()
    ...
    def validation_epoch_end(self, outputs):
        ...
        if self.config.get('log1p_label', True):
        ...
        if hasattr(self, '_train_preds_accumulator') and self._train_preds_accumulator and self._train_truths_accumulator:
            ...
            if self.config.get('log1p_label', True):
                train_truths = torch.expm1(train_truths)
