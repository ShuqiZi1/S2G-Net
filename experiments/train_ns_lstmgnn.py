...
def main(config):
    ...
    for phase in ['test', 'valid']:
        ...
        if 'per_node' in ret:
            ...
            try:
                if phase == 'valid':
                    all_preds = torch.cat(model.val_preds, dim=0)
                    all_preds_lstm = torch.cat(model.val_preds_lstm, dim=0)
                    all_truths = torch.cat(model.val_truths, dim=0)
                else:
                    all_preds = torch.cat(model.test_preds, dim=0)
                    all_preds_lstm = torch.cat(model.test_preds_lstm, dim=0)
                    all_truths = torch.cat(model.test_truths, dim=0)
                
                per_node_gnn = model.per_node_metrics(all_truths, all_preds)
                per_node_lstm = model.per_node_metrics(all_truths, all_preds_lstm)
                per_node_lstm = {n + '_lstm': per_node_lstm[n] for n in per_node_lstm}
                per_node = {**per_node_gnn, **per_node_lstm}
